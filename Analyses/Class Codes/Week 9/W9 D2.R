...
#Equal Slopes Model
eqslopes <-lm(mpg ~ qsec + am, data=mtcars)
summary(eqslopes)

#Equal Intercepts Model
eqint <-lm(mpg ~ qsec + qsec:am, data=mtcars)
summary(eqint)
...
b <- coef(lm.2lines)
curve(b[1] + b[2]*x, col="yellow", lwd=2, add=TRUE)  #baseline (in blue)
curve((b[1] + b[3]) + (b[2] + b[4])*x, col="red", lwd=2, add=TRUE) #changed line (in orange)
b
#Equal Slopes Model - redo graph first
b <- coef(eqslopes)
curve(b[1] + b[2]*x, col="yellow", lwd=2, add=TRUE)  #baseline (in blue)
curve((b[1] + b[3]) + (b[2])*x, col="red", lwd=2, add=TRUE) #changed line (in orange)
#Equal Intercepts Model  - redo graph first
b <- coef(eqint)
curve(b[1] + b[2]*x, col="yellow", lwd=2, add=TRUE)  #baseline (in blue)
curve((b[1]) + (b[2] + b[3])*x, col="red", lwd=2, add=TRUE) #changed line (in orange)
# Then b will have 4 estimates:
# b[1] is the estimate of beta_0: -9.0099
# b[2] is the estimate of beta_1:  1.4385
# b[3] is the estimate of beta_2: -14.5107
